...
async function Board({ locale, searchParams }: { locale: Locale; searchParams: SP }) {
  ...
  const filtersActive = query.sort !== DEFAULT_SORT || query.band !== "all" || query.topic !== null;
  const tab = (on: boolean) =>
    `relative flex h-14 shrink-0 items-center gap-2 px-4 text-[14px] font-medium ${on ? "text-fg after:absolute after:inset-x-4 after:bottom-0 after:h-0.5 after:bg-accent" : "text-muted hover:text-fg"}`;
  const count = (n: number | undefined) =>
    n === undefined ? null : <span className="rounded-full bg-fg/10 px-2 text-[12px] font-medium leading-6">{n}</span>;
  const chip = (on: boolean) =>
    `rounded-full border px-3 py-1 text-[13px] ${on ? "border-accent bg-accent/15 text-fg" : "border-line text-muted hover:text-fg"}`;

  return (
    <div>
      {/* Tab row (online bands) with the filter and search icons at the right, Holodex-style. */}
      <div className="sticky top-14 z-20 border-b border-line bg-bg/95 backdrop-blur">
        <div className="flex flex-wrap items-center px-2">
          ...
          <BoardFilters active={filtersActive}>
      ...
      <div className="px-4 py-4">
        ...
        {oc && oc.unknown > 0 && query.online !== null ? <p className="mb-2 text-xs text-faint">{fmt(m.board.unknownOnline, { n: oc.unknown })}</p> : null}
