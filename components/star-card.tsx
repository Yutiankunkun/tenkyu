...
/**
 * One room, Holodex card anatomy: 16:9 thumbnail with topic chip (top-left), ♡ (top-right),
 * elapsed badge (bottom-right); below it avatar, two-line title, streamer, status line.
 */
export function StarCard({ r, now }: { r: StarRow; now: number }) {
  const { locale, m } = useI18n();
  const href = localePath(locale, `/watch/${r.room_id}`);
  ...
  return (
    <article className="group flex flex-col">
      <div className="relative aspect-video overflow-hidden rounded bg-surface">
        <Link href={href} className="absolute inset-0" aria-label={r.title}>
          {r.cover ? (
      ...
      <div className="flex gap-3 pt-2">
        <Link href={href} className="shrink-0 pt-0.5">
          <Avatar src={r.bili_streamer.face} name={r.bili_streamer.uname} size={40} />
        </Link>
        <div className="min-w-0 flex-1">
          <Link href={href} className="clamp-2 text-[14px] font-medium leading-snug text-fg hover:underline">
